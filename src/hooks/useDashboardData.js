...
export function useDashboardData(symbol, interval) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const wsUrl = `wss://stream.binance.com:9443/stream?streams=${symbol.toLowerCase()}@ticker/${symbol.toLowerCase()}@kline_${interval}`;
    ...
    return () => {
      ...
      if (ws) {
        // Only close if the socket is CONNECTING (0) or OPEN (1).
        // Closing a socket that's already CLOSING/CLOSED causes the
  ...
  return { data, ticker, fng, loading, payload };
}